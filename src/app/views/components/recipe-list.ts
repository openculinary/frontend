...
function titleFormatter(recipe: types.Recipe) : JQuery {
...
function recipeFormatter(value: HTMLElement, recipe: types.Recipe) : HTMLElement {
  const container = $('<div />');
...
function updateRecipeState(recipeId: string) : void {
  db.recipes.get(recipeId, (recipe?: types.Recipe) => {
...
function updateStarState(selector: string, recipeId: string) : void {
  db.starred.get(recipeId, (starred?: types.Starred) => {

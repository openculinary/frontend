...
async function getRecipeById(recipeId: string) : Promise<types.Recipe> {
  return await db.recipes.get(recipeId, async (recipe: types.Recipe) => {
...
async function getRecipe(el: HTMLElement) : Promise<types.Recipe> {
  let recipe: types.Recipe = null;
  const target = $(el).hasClass('recipe') ? $(el) : $(el).parents('.recipe');
